...
export default function VerifyClient() {
  ...
  useEffect(() => {
    const verifyEmail = async () => {
      ...
      try {
        const res = await fetch(`/api/register?token=${id}`);
        ...
        setStatus("error");
        setMessage(error + "Une erreur est survenue lors de la vérification.");
      }
    };
  ...
  if (status === "success") return <p style={{ color: "green" }}>{message}</p>;
